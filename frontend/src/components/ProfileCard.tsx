...
interface Props {
  locale: LocaleConfig;
  data: ParseResponse;
  source: 'live' | 'mock-fallback' | 'demo';
}

const SOURCE_BADGE: Record<Props['source'], { label: string; tone: string }> = {
  ...
  return (
    ...
    >
      ...
      <header className="relative bg-gradient-to-br from-sun-400/30 to-clay-200/30 px-5 py-5 sm:px-7 sm:py-6">
        ...
        <div className="mt-3 flex flex-wrap items-center gap-2">
          ...
          <span className="pill bg-clay-100 text-clay-700">
            <span aria-hidden>⚡</span> {latency_ms} ms
          </span>
      ...
      <div className="grid gap-5 px-5 py-5 sm:grid-cols-2 sm:px-7 sm:py-6">
        <section aria-label="Languages and skills">
          ...
          <h3 className="mt-5 text-sm font-semibold text-clay-800">
          ...
          <ul className="mt-1.5 space-y-1.5">
            {profile.skills.map((s) => (
              <li
                key={s.name}
                className="flex items-baseline justify-between gap-3 rounded-lg bg-clay-50 px-3 py-2"
              >
                <div className="min-w-0">
                  <p className="truncate text-sm font-medium text-clay-900">{s.name}</p>
                  {s.evidence && (
                    <p className="truncate text-xs text-clay-600">“{s.evidence}”</p>
                  )}
                </div>
        ...
        <section aria-label="Signals and network entry" className="space-y-5">
          <SignalBar label="Wage signal" signal={profile.wage_signal} tone="wage" />
          <SignalBar label="Growth signal" signal={profile.growth_signal} tone="growth" />
